...
interface IConcert
{
    ...
    class BookingSystem
    {
        ...
        public void Szukaj()
        {
            ...
            switch (wybor)
            {
                ...
                case "2":
                    ...
                    break;
                case "3":
                    Console.WriteLine("Podaj max cene [zl]");
        ...
        public void Rezerwacja()
        {
            ...
            string concertNamee = Console.ReadLine();
            foreach (var concert in concerts)
            {
                if (concertNamee == concert.Name)
                {
        ...
        public void Wyswietl()
        {
            ...
            foreach (var concert in concerts)
            {
                ...
                {
                    Console.WriteLine(
                        $"{concert.Name} - {concert.Date} w {concert.Location} ({concert.AvailableSeats} miejsc dostępnych, cena: {concert.TicketPrice} Spotkania: {vipConcert.Spotkanie}");
                }
                else if (concert is OnLineConcert onLineConcert)
                {
                    Console.WriteLine(
                        $"{concert.Name} - {concert.Date} w {concert.Location} ({concert.AvailableSeats} miejsc dostępnych, cena: {concert.TicketPrice} Platforma: {onLineConcert.Platforma})");
                }
                else if (concert is PrivateConcert privateConcert)
                {
                    Console.WriteLine(
                        $"{concert.Name} - {concert.Date} w {concert.Location} ({concert.AvailableSeats} miejsc dostępnych), cena: {concert.TicketPrice} Zaproszenie: {privateConcert.Zaproszenie}");
                }
                else if (concert is RegularConcert regularConcert)
                {
                    Console.WriteLine(
                        $"{concert.Name} - {concert.Date} w {concert.Location} ({concert.AvailableSeats} miejsc dostępnych, cena: {concert.TicketPrice})");
                }
                else
                {
